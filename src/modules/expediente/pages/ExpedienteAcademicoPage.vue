<template>
  <DashboardLayout>
    <div class="p-4 space-y-4">
      <!-- Selector de periodo -->
      <CustomPeriodSelector :periods="filteredPeriods" :modelValue="selectedPeriod" @update-modelValue="updatePeriod" />

      <!-- Acordeón que contiene la tabla de datos -->
      <CustomAccordion title="Detalles del Expediente">
        <CustomDataTable :data="filteredData" :columns="columns" :actionsVisible="true" @edit-item="openEditModal" />
      ...
      <!-- Modal de Edición -->
      <EditModal :isVisible="isModalVisible" :expediente="selectedExpediente" @update:expediente="updateExpediente" @close="closeModal" />
    </div>
  </DashboardLayout>
</template>
...
const filteredData = computed(() => {
  const filteredDataTmp = data.value.filter((item) => item.period === selectedPeriod.value)
  console.log("filteredDatTmp", filteredDataTmp)
